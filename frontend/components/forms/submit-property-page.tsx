...
export function SubmitPropertyPage() {
  ...
  useEffect(() => {
    const fetchFeaturedPlans = async () => {
      try {
        const response = await apiClient.get<any>('/api/content/featured-plans');
        if (response.success) {
  ...
  const uploadPaymentProof = useCallback(async (file: File) => {
    ...
    try {
      const response = await apiClient.post<any>('/api/properties/upload-images', formData, {
        headers: {
    ...
    try {
      ...
      if (!finalCoords) {
        try {
          ...
          console.error("Geocoding failed for full address:", e);
        }
      }

      if (!finalCoords && form.city) {
        try {
          finalCoords = await geocodeProperty(`${form.city}, ${form.state}, India`, "");
        } catch (e) {
      ...
      if (!finalCoords) {
        finalCoords = { lat: 18.5204, lng: 73.8567 }; // Default coordinates (Pune/India)
      }
      ...
      if (!finalAmenities) {
        try {
          ...
          console.error("Failed to generate nearby amenities:", e);
          finalAmenities = {
            hospital: null,
            school: null,
            supermarket: null,
      ...
      // Submit property to backend API
      await estateApi.properties.submit(propertyPayload);
